...
public class FileUtils {
	...
	public static String getRootPath11(Context context, String dirName) {
		String rootPath = null;
		if (Environment.getExternalStorageState().equals(Environment.MEDIA_MOUNTED)) {

			rootPath = Environment.getExternalStorageDirectory().getAbsolutePath() + File.separator
					+ context.getPackageName() + File.separator + "物证元素" + File.separator + dirName + File.separator;
		} else {
			rootPath = context.getCacheDir().getAbsolutePath() + File.separator + "物证元素" + File.separator + dirName
					+ File.separator;
		}
		File dir = new File(rootPath);
		dir.mkdirs();
